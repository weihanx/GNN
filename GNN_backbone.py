...
class HeteroGNN(torch.nn.Module):
    def __init__(self, num_input_features, hidden_channels, out_channels, normalize=False):
        ...
        self.conv_1 = HeteroConv({
            ...
            ('note', 'up', 'note'): GCNConv(num_input_features, hidden_channels, normalize),
            ('note', 'down', 'note'): GCNConv(num_input_features, hidden_channels, normalize),
        }, aggr='sum')
        ...
        self.conv_2 = HeteroConv({
            ...
            ('note', 'up', 'note'): GCNConv(hidden_channels, hidden_channels, normalize),
            ('note', 'down', 'note'): GCNConv(hidden_channels, hidden_channels, normalize),
        }, aggr='sum')
