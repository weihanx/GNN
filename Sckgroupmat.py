...
class GroupMat(torch.nn.Module):
    def __init__(self, num_features, embedding_dim = 5, hidden_dim = 200, num_classes=15, device="cuda"):
        # cat_cardinalities
        """
        cat_cardinalities = [
    ...
    def forward(self, data):
        ...
        batch = data['note'].batch
        num_nodes = x['note'].shape[0]
        # print(f"num nodes = {num_nodes}")
        attribute_dict = {}
        for edge_type in data.edge_types:
            attribute_dict[edge_type] = data[edge_type].edge_attr
        x['note'] = x['note'].float()
        # pitch_features, midi_features, duration_features, normalized_offsetlist, ocativelist
        # x['note']: model(x_cont,x_cat): 
        # x_cat = x['note'][:,:3] 
        # x_cont = x['note'][:, 3:]
        # n_cat_features = len(self.cat_cardinalities)
        # x_cat = torch.column_stack([
        #     torch.randint(0, c, (num_nodes,))
        #     for c in self.cat_cardinalities
        # ])
        # print(f"shape of categorical = {x_cat.shape}, x_cont = {x_cont.shape}")
        # with torch.no_grad():
        # x_cat = self.embed(x['note'][:,:-2])
        # x_cont = x['note'][:,-2:]
