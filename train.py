...
SAVE_FOLDER = "hetergraph0422_6feature_6edge_Weighted/"
...
def train_sck(model, loss_fn, train_loader, class_weight):
    ...
    for databatch in train_loader:
        ...
        s1 = s1.to("cuda")
        s2 = s2.to("cuda")  
        # print(f"s1_pred = {s1_pred}, s1 = {s1}")
        # print(f"s2+pred = {s2_pred}, s1 = {s2}")
        ...
        # loss2 = F.cosine_similarity(torch.flatten(s1_pred.unsqueeze(0), 1), torch.flatten(s1.unsqueeze(0), 1))
        # loss3 = F.cosine_similarity(torch.flatten(s2_pred.unsqueeze(0), 1), torch.flatten(s2.unsqueeze(0), 1))
...
train_acc_curve = []
valid_acc_curve = []
loss_fn = WEIGHTED_SIM_CRITERION
